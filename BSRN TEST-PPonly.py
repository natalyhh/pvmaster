# STEP 2.3 - BSRN TEST - Only physically possible - 1 MINUTE DATA - DESCRIPTION AT THE END OF PAGE - JULY / 2021
...
ghiflagcount = pd.DataFrame(columns= [ghi], index = [-1, 0, 1, 2], dtype = int) 
difflagcount = pd.DataFrame(columns= [dif], index = [-1, 0, 1, 2], dtype = int) 
dniflagcount = pd.DataFrame(columns= [dni], index = [-1, 0, 1, 2], dtype = int) 


#### end definitions

for year in years:
    ghiFLAG = pd.DataFrame(columns = ['PPmax', 'min', 
                                      'F_GHIP_SI02pt100', 'F_GHIP_SMP11', 
                                      'F_GHIP_SMP22', 'F_GHIP_SPN1', 'F_GHIT_SMP22'])
    difFLAG = pd.DataFrame(columns = ['PPmax', 'min', 
                                      'F_DfHIP_SPN1', 'F_DfHIRP_SMP11', 
                                      'F_DfHIT_SMP22', 'F_DIF_GLO_REF_SPN1'])  
    dniFLAG = pd.DataFrame(columns = ['PPmax', 'min', 'F_DNI_SHP1'])
