#STEP 2.1 - BSRN TEST - 1SECOND DATA - DESCRIPTION AT THE END OF PAGE - JULY / 2021
...
BSRN_path = "/Users/nataly/opt/AnacondaProjects/SAPIENS/BSRNsecin/" 
...
for year in years:
    ...
    dfghi.index = dfghi.index.tz_localize(location.tz) # NEED TO FIX THIS SO EVERYTHING IS SAME TZ
    ...
    #ghiFLAG = pd.merge(ghiFLAG, dfghi, left_index = True, right_index = True, how = 'inner')  # index by naive times. how = 'inner': index by recorded times.
    #difFLAG = pd.merge(difFLAG, dfdif, left_index = True, right_index = True, how = 'inner')  # index by naive times. how = 'inner': index by recorded times.
    #dniFLAG = pd.merge(dniFLAG, dfdni, left_index = True, right_index = True, how = 'inner')  # index by naive times. how = 'inner': index by recorded times.
    
    ghiFLAG = ghiFLAG.join(dfghi, how = 'left')  # index by naive times. how = 'inner': index by recorded times.
    print('JOIN ghiFLAG.shape =', ghiFLAG.shape,year)

    #difFLAG = difFLAG.join(dfdif, how = 'left')  # index by naive times. how = 'inner': index by recorded times.
    #dniFLAG = dniFLAG.join(dfdni, how = 'left')  # index by naive times. how = 'inner': index by recorded times.
